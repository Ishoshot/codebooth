<script lang="ts">
  ...
  import Follow from "./util/Follow.svelte";
  ...
  async function getUserInfo() {
    const response = await fetch(`${apiBaseURL}/users/${userID}/info`, {
      method: "GET",
      headers: {
  ...
  onMount(async () => {
    window.addEventListener("message", async (event) => {
      ...
      switch (message.type) {
        /* --------------------- Accept Token and getUserInfo() -------------------- */
        case "token": {
          userID = message.value.split("/")[0];
          accessToken = message.value.split("/")[1];
          getUserInfo();
          break;
        }
      }
